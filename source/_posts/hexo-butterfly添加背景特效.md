...
{% note blue 'fas fa-bullhorn' flat %}收集好看的特效，这些是比较符合我心意的，如果还找到其它的，我会继续更新到下面{% endnote %}

{% note simple %}新建`themes\butterfly\source\js`目录下，自定义文件夹保存背景特效（大部分都是取名困难症，直接用别人的了）。{% endnote %}
...
这个直接引入到`_config.butterfly.yml`文件中,放到`bottom`下。
...
在主题配置文件`_config.butterfly.yml`,在`inject`的`bottom`下放入控制文件（两个选择，自行斟酌）。
