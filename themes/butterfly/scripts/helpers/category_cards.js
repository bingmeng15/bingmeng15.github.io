hexo.extend.helper.register('category_cards', function (type) {
    ...
    hexo.locals.get(type).map(function (item) {
        ...
        if (eval('conf.' + type) && eval('conf.' + type + '.' + item.name)) {
            ...
            var itemclo1 = eval('conf.' + type + '.' + item.name + '.color1') ? eval('conf.' + type + '.' + item.name + '.color1') : '#67c2ff' //默认颜色1
            var itemclo2 = eval('conf.' + type + '.' + item.name + '.color2') ? eval('conf.' + type + '.' + item.name + '.color2') : '#006aff' //默认颜色2
        }
        else {
            var itemdesc = ''
            var itemimg = item.posts.data[0].cover
            var itemclo1 = '#67c2ff' //默认颜色1
            var itemclo2 = '#006aff' //默认颜色2
        }
